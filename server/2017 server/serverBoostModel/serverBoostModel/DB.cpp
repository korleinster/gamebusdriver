...
bool DB::DB_Login(wchar_t* id, wchar_t* pw, player_session *player) {
	...
	SQLLEN pIndicators[8];		// DB 상 안에 있는 목차 갯 수

	if (SQLExecDirect(hStmt, (SQLWCHAR*)L"SELECT RTRIM(ID), RTRIM(Password), RTRIM(Nickname), PlayerLevel, Admin, Queststate, PositionX, PositionY FROM dbo.user_data", SQL_NTS) != SQL_ERROR) {
		// 숫자 1 은, 첫번째 파라미터 ID
		...
		SQLBindCol(hStmt, 3, SQL_C_WCHAR, (SQLPOINTER)player->get_player_data()->nickname, MAX_BUF_SIZE / 4, &pIndicators[2]);
		SQLBindCol(hStmt, 4, SQL_C_STINYINT, (SQLPOINTER)&player->get_player_data()->state.level, sizeof(char), &pIndicators[3]);
		SQLBindCol(hStmt, 5, SQL_C_BIT, (SQLPOINTER)&Admin, sizeof(Admin), &pIndicators[4]);
		SQLBindCol(hStmt, 5, SQL_C_LONG, (SQLPOINTER)&player->get_sub_data()->quest, sizeof(int), &pIndicators[5]);	// quest
		SQLBindCol(hStmt, 7, SQL_C_FLOAT, (SQLPOINTER)&player->get_player_data()->pos.x, sizeof(float), &pIndicators[6]);
		SQLBindCol(hStmt, 8, SQL_C_FLOAT, (SQLPOINTER)&player->get_player_data()->pos.y, sizeof(float), &pIndicators[7]);

		while (SQLFetch(hStmt) == SQL_SUCCESS)
		{
			if ((wcscmp(input_id, id) == 0) && (wcscmp(input_pw, pw) == 0)) {
				// 확인이 되었다면, 여기서 데이터를 복사해야 하는데..
				if (NULL == player->get_sub_data()->quest) { player->get_sub_data()->quest = 0; }
				if (NULL == player->get_player_data()->pos.x) { player->get_player_data()->pos.x = 160; }
				if (NULL == player->get_player_data()->pos.y) { player->get_player_data()->pos.y = 400; }
...
bool DB::DB_Update(wchar_t * id, player_session * ps)
{
	wchar_t query[1000] = { 0 };
	_stprintf(query, L"UPDATE dbo.user_data SET Queststate = %d, PositionX = %f, PositionY = %f WHERE ID = '%s'", ps->get_sub_data()->quest, ps->get_player_data()->pos.x, ps->get_player_data()->pos.y, id);
	//wsprintfW(query, L"UPDATE dbo.user_data SET Queststate = '%d', PositionX = '%f', PositionY = '%f' WHERE ID = '%s'", ps->get_sub_data()->quest, ps->get_player_data()->pos.x, ps->get_player_data()->pos.y, id);

	if (SQLExecDirect(hStmt, (SQLWCHAR*)query, SQL_NTS) != SQL_ERROR) {
