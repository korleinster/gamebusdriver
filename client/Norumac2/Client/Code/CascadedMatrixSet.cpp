...
CCascadedMatrixSet::CCascadedMatrixSet() : m_bAntiFlickerOn(true), m_fCascadeTotalRange(10.0f), m_pCamera(CCamera::GetInstance())
{
...
void CCascadedMatrixSet::Update(const D3DXVECTOR3& vDirectionalDir)
{
	...
	D3DXVECTOR3 vWorldCenter = m_pCamera->m_vEye + m_pCamera->GetWorldAhead() * m_fCascadeTotalRange * 0.5f;
	D3DXVECTOR3 vPos = vWorldCenter;
	D3DXVECTOR3 vLookAt = vWorldCenter + vDirectionalDir * m_pCamera->m_fFar;
...
void CCascadedMatrixSet::ExtractFrustumPoints(float fNear, float fFar, D3DXVECTOR3* arrFrustumCorners)
{
	...
	const D3DXVECTOR3& camPos = m_pCamera->m_vEye;//*g_Camera.GetEyePt();
	const D3DXVECTOR3& camRight = m_pCamera->GetWorldRight();
	const D3DXVECTOR3& camUp = m_pCamera->GetWorldUp();
	const D3DXVECTOR3& camForward = m_pCamera->GetWorldAhead();

	// Calculate the tangent values (this can be cached
	const float fTanFOVX = tanf(m_pCamera->m_fAspect * m_pCamera->m_fFovY);
	const float fTanFOVY = tanf(m_pCamera->m_fAspect);
...
{
	// Get the camera bases
	const D3DXVECTOR3& camPos = m_pCamera->m_vEye;
	const D3DXVECTOR3& camRight = m_pCamera->GetWorldRight();
	const D3DXVECTOR3& camUp = m_pCamera->GetWorldUp();
	const D3DXVECTOR3& camForward = m_pCamera->GetWorldAhead();
...
bool CCascadedMatrixSet::CascadeNeedsUpdate(const D3DXMATRIX& mShadowView, int iCascadeIdx, const D3DXVECTOR3& newCenter, D3DXVECTOR3& vOffset)
{
	...
	bool bNeedUpdate = abs((int)(fPixelOffX)) > 0.5f || abs((int)(fPixelOffY)) > 0.5f;
	if(bNeedUpdate)
	{
		// Round to the 
