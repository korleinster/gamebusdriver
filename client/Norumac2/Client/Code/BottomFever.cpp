...
HRESULT CBottomFever::Initialize(void)
{
...
CBottomFever * CBottomFever::Create(void)
{
	CBottomFever* pUI = new CBottomFever;
...
HRESULT CBottomFever::AddComponent(void)
{
	...
	pComponent = CResourcesMgr::GetInstance()->CloneResource(RESOURCE_STAGE, L"Texture_BottomFever");
...
void CBottomFever::UpdateBufferToFever(void)
{
	...
	wsprintf(wcFever, L"%d", (*player)->GetPacketData()->state.gauge);

	m_pFont->m_wstrText = wcFever;

	m_fY = m_fOriginY;
	m_fY += ((1.f - fFever) * m_fSizeY * 0.025f);

	VTXTEX vtx[] =
	{
		{ D3DXVECTOR3(-1.f, 1.f - (fFever * 2.f), 0.f),D3DXVECTOR3(0.f, 1.f, 0.f) , D3DXVECTOR2(0.f, 0.f + fFever) },
		{ D3DXVECTOR3(1.f , 1.f - (fFever * 2.f), 0.f) , D3DXVECTOR3(0.f, 1.f, 0.f), D3DXVECTOR2(1.f , 0.f + fFever) },
